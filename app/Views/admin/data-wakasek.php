                ...
                <div class="modal-body">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-10">
                            ...
                            <div class="errorjabatan invalid-feedback"></div>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    <input type="hidden" name="aksi" id="aksi">
                    <input type="hidden" name="idwakasek" id="idwakasek">
...
<script>
    $(document).ready(function(){
        ...
        $(document).on('click','.btn-add',function(e){
            e.preventDefault();
            $('#formTambah')[0].reset();
            $('.modal-title').html('Tambah Data Wakasek')
            $('.btnSave').html('Tambah')
            $('#aksi').val('insert')
        })
        ...
        $(document).on('click','.btn-edit',function(e){
            ...
            var id = $(this).attr('id')
            $.ajax({
                url : '/admin/data-wakasek',
                data : {id:id,_method:'PUT'},
                method : 'post',
                dataType : 'json',
                success : function(data){
                    console.log(data);
                    $('#idwakasek').val(data.id_wakasek)
                    $('#lembaga').val(data.id_lembaga)
                    $('#guru').val(data.id_guru)
                    $('#jabatan').val(data.jabatan)
                    $('.modal-title').html('Edit Data Penugasan')
                    $('.btnSave').html('Edit')
                    $('#aksi').val('update')
                }
            })
                ...
                success : function(data){
                    ...
                    if(data.error){
                        if(data.error.lembaga){
                            ...
                            $('.errornama').html(data.error.lembaga)
                        }else{
                        ...
                        if(data.error.jabatan){
                            ...
                            $('.errorjabatan').html(data.error.jabatan)
                        }else{
